package  
{
	...
	public class PlayerClass extends MovieClip
	{
		...
		{
			addChild(run);//run anim
			addChild(slide);//slide anim
			addChild(jumpanim);//jump anim
		...
		public function jump():void{
			jumpSpeed = -30; //30
			isJumping = true;//can jump
			playAnim(PlayerClass.JUMP);//jump
		...
		public function Slide():void
		{
			...
			{
				playAnim(PlayerClass.SLIDE);//slide
				
			}			
			if (!sliding && !run.visible && !isJumping)
			{
				playAnim(PlayerClass.RUN);//run
			}
